...
void PrintContacts(Contacts&contacts){
    ...
    for(int i=0;i<contacts.contacts_size();++i){
      ...
      const UnknownFieldSet &set=reflection->GetUnknownFields(contact);
      for(int j=0;j<set.field_count();++j){
          auto unknow_file=set.field(j);
          cout<<"未知字段:"<<j+1<<endl;
        switch(unknow_file.type()){
            case google::protobuf::UnknownField::TYPE_VARINT:
            cout<<"值:"<<unknow_file.varint();
            break;
            case google::protobuf::UnknownField::TYPE_FIXED32:
            cout<<"值:"<<unknow_file.fixed32()<<endl;
            //case
        }
      }
